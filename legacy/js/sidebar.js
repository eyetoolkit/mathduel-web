...
(function(){
  ...
  var SITE_CONFIG = {
    board: {
      name: 'Board Duel', nameZh: '棋盘对决', emoji: '⚔️', host: 'boardduel.com',
      games: [
        { icon: '✕◯',  name: 'Tic-Tac-Toe',  zh: '井字棋',   href: '/games/tictactoe/' },
        { icon: '⚫',  name: 'Gomoku',       zh: '五子棋',   href: '/games/gomoku/' },
        { icon: '🔴',  name: 'Connect 4',    zh: '四子棋',   href: '/games/connect4/' },
        { icon: '◐',   name: 'Othello',      zh: '黑白棋',   href: '/games/othello/' },
        { icon: '♞',   name: 'Chess',        zh: '国际象棋', href: '/games/chess/' },
        { icon: '⛀',   name: 'Checkers',     zh: '西洋跳棋', href: '/games/checkers/' }
      ]
    },
    math: {
      name: 'Math Duel', nameZh: '数学对决', emoji: '🔢', host: 'mathduel.games',
      games: [
    ...
    memory: {
      name: 'Memory Duel', nameZh: '知识对战', emoji: '🧠', host: 'memoryduel.com',
      games: [
  ...
  function applySidebarLang(lang){
    ...
    // Game items — 用 SITE_CONFIG 里的 zh/name 字段
    var gameLinks = document.querySelectorAll('[data-sb-game]');
    gameLinks.forEach(function(a){
      var idx = parseInt(a.dataset.sbGame, 10);
      if (!isNaN(idx) && cfg.games[idx]) {
        a.textContent = lang === 'zh' ? cfg.games[idx].zh : (cfg.games[idx].name || cfg.games[idx].zh);
      }
    });
    // 语言按钮 label（三语言完整）
  ...
  function buildSidebarHTML(){
    ...
    var NAV_LABELS = NAV_I18N[getSiteLang()] || NAV_I18N['zh'];
    ...
    return '' +
      '<aside class="sb" id="triSidebar">' +
        ...
        '<nav class="sb-body" aria-label="Main navigation">' +
          '<div class="sb-group">' +
            '<a class="sb-link'+(currentPath==='/' ? ' active' : '')+'" href="/"><span class="sb-icon">🏠</span><span data-sb-nav="home">'+NAV_LABELS.home+'</span></a>' +
          '</div>' +
          '<div class="sb-group">' +
            '<div class="sb-group-title" data-sb-nav="games-group">'+NAV_LABELS.games+'</div>' +
            gamesHTML +
          '</div>' +
          '<div class="sb-group">' +
            '<a class="sb-link" href="/about/"><span class="sb-icon">ℹ️</span><span data-sb-nav="about">'+NAV_LABELS.about+'</span></a>' +
          '</div>' +
        '</nav>' +
        '<div class="sb-footer">' +
          '<div class="sb-lang" id="sbLang">' +
            '<button class="sb-lang-btn" id="sbLangBtn" type="button" aria-haspopup="listbox" aria-expanded="false">' +
              ...
              '<span id="sbLangLabel">'+NAV_LABELS.langLabel+'</span>' +
              '<svg class="sb-caret" viewBox="0 0 12 12" aria-hidden="true"><path d="M2 4l4 4 4-4" stroke="currentColor" fill="none" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg>' +
            '</button>' +
      ...
      '<button class="sb-toggle" id="sbToggle" type="button" aria-label="'+NAV_LABELS.langLabel+'" aria-expanded="false">' +
  ...
  function bindSidebarLang(){
    ...
    var LANG_LABELS = {
      en: 'English' // TEMP: multilingual disabled
    };
