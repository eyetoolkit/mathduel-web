(function(){
  ...
  var MONTH_LABELS_KEYS = { 1: 'coins.month_1', 3: 'coins.month_3', 6: 'coins.month_6', 12: 'coins.month_12' };
  ...
  function renderContent(d) {
    ...
    if (adActive) {
      html += '<div style="background:#dcfce7;border:1px solid #86efac;color:#166534;padding:12px;border-radius:10px;margin-bottom:16px;font-weight:600;text-align:center">\u2705 ' + __('coins.ad_free_active', '免广告已激活，剩余').replace('{days}', adDays) + ' <b>' + adDays + '</b> ' + __('common.days', '天') + '</div>';
    }
    html += '<div style="background:#f9fafb;border-radius:10px;padding:16px;margin-bottom:16px;text-align:center">';
    html += '<div style="font-size:12px;color:#6b7280;margin-bottom:4px">' + __('coins.current_balance', '' + T('coins.current_balance','Current points') + '') + '</div>';
    ...
    for (var m in PRICES) {
      ...
      var statusHtml = canAfford
        ? '<span style="color:#16a34a;font-weight:600">\u2713 ' + __('coins.can_exchange', '' + T('coins.can_exchange','Available') + '') + '</span>'
        : '<span style="color:#dc2626;font-size:12px">' + __('coins.deficit', '差').replace('{n}', deficit.toLocaleString()) + ' ' + deficit.toLocaleString() + '</span>';
      var labelKey = MONTH_LABELS_KEYS[m] || 'coins.month_other';
      html += '<button class="exchange-btn" data-months="' + m + '" style="' +
        ...
        '<div><span style="font-weight:700;font-size:16px;color:#1f2937">' + __(labelKey, LABELS[m]) + '</span><br>' +
        '<span style="color:#6b7280">\uD83E\uDE99 ' + price.toLocaleString() + ' ' + __('coins.points', '积分') + '</span></div>' +
        '<div>' + statusHtml + '</div></button>';
    }
    html += '</div>';
    html += '<div style="margin-top:14px;padding:10px;background:#fef9c3;border-radius:8px;font-size:12px;color:#854d0e;text-align:center">\u26A1 ' + __('coins.invite_bonus', '' + T('coins.invite_bonus','Invite a friend, earn 500 points!') + '').replace('{link}', '[ ' + __('coins.invite_now', '立即邀请') + ' ]') + '<br><a href="#" id="go-invite" style="color:#ca8a04;font-weight:600">[' + __('coins.invite_now', '立即邀请') + ']</a></div>';
    html += '</div>';
    return html;
  }
  ...
  function refresh(cb) {
    fetch('/api/coins/me', { credentials: 'same-origin' })
  ...
  function doPurchase(months) {
    ...
    if (btn) { btn.disabled = true; btn.textContent = __('common.processing', '' + T('common.loading','Processing...') + ''); }

    fetch('/api/coins/purchase-adfree', {
      method: 'POST',
      ...
      .then(function(d){
        if (d && d.ok) {
          ...
          var html = '<div style="text-align:center;padding:24px 0">' +
            ...
            '<div style="font-size:20px;font-weight:700;color:#1f2937;margin-bottom:8px">' + __('coins.exchange_success', '' + T('coins.exchange_success','Exchange successful!') + '') + '</div>' +
            '<div style="color:#6b7280;margin-bottom:20px">' + __('coins.ad_free_remaining', '免广告剩余').replace('{days}', days) + ' <b style="color:#4f46e5">' + days + '</b> ' + __('common.days', '天') + '</div>' +
            '<div style="font-size:14px;color:#6b7280">' + __('coins.remaining_balance', '' + T('coins.remaining_balance','Remaining points: ') + '').replace('{n}', '') + '\uD83E\uDE99 <b>' + d.coins.toLocaleString() + '</b></div>' +
            '<button onclick="CoinExchange.close()" style="margin-top:20px;padding:10px 32px;background:#4f46e5;color:#fff;border:none;border-radius:8px;cursor:pointer;font-size:14px;font-weight:600">' + __('common.got_it', '知道了！') + '</button>' +
            '</div>';
          var panel = qs('#exchange-panel');
          if (panel) panel.innerHTML = html;
          if (window.CoinsBar && window.CoinsBar.refresh) window.CoinsBar.refresh();
        } else {
          var errMsg = { insufficient_coins: __('coins.insufficient_balance', '' + T('coins.insufficient_balance','Insufficient points') + '') };
          alert((errMsg[d.error] || d.error || __('coins.exchange_failed', '' + T('coins.exchange_failed','Exchange failed') + '')) + '！');
          state.loading = false;
          refresh(function(dd){ var p = qs('#exchange-panel'); if (p) p.innerHTML = renderContent(dd); });
        }
      })
      .catch(function(){
        alert(__('common.network_error', '' + T('common.network_error','Network error, please retry!') + ''));
        state.loading = false;
        refresh(function(dd){ var p = qs('#exchange-panel'); if (p) p.innerHTML = renderContent(dd); });
      });
  }
