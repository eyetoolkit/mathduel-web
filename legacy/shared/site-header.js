...
/* ── 裸域归一化（临时的客户端兜底）─────────────────────────────
   背景：worker 路由只配了 mathduel.games/api/*，没有 www.mathduel.games/*，
   所以从 www 打开站点时 api/* 会落到 Pages，POST /api/rooms 返回 405，
   前端表现为 "Connection failed, please retry"（建房/入房都失败）。
   处理：把 www 跳回裸域。这同时符合 canonical（全站 canonical 均指向裸域）。
   根治：给 CF token 补 "Workers Routes: Edit" 权限后，由 wrangler.toml 里
   已声明的 www.mathduel.games/api/* 与 /ws* 路由接管，届时可删掉这段。 */
(function () {
  try {
  ...
  ready(function () {
    ...
    var GAMES = {
      ...
      math: [
        ...
        { key: "nav.game_bulls", label: "Number Guess", href: "/games/bulls/" }
      ],
      memory: [
